...
def _pathogenicity_spectrum_badge(r):
    ev = r.get("evidence", {}) or {}
    cv_sig = str(ev.get("clinvar_class") or r.get("clinvar_sig") or "").lower()
    
    def _float(val):
        try: return float(val)
        except (TypeError, ValueError): return None

    revel = _float(r.get("revel"))
    am_score = _float(r.get("am_path") or r.get("alphamissense"))
    am_class = str(r.get("am_path") or r.get("alphamissense") or "").lower()
    cadd = _float(r.get("cadd_phred"))
    vest = _float(r.get("vest4") or r.get("vest4_score"))
    chasm = _float(r.get("chasmplus") or r.get("chasmplus_score"))
    
    if "protective" in cv_sig:
        return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-bold bg-teal-100 text-teal-900 border border-teal-300" title="ClinVar Protective Allele">'
                '💚 Protective</span>', 'protective')
                
    if ("pathogenic" in cv_sig and "likely" not in cv_sig) or (revel is not None and revel >= 0.75) or (am_score is not None and am_score >= 0.80) or "pathogenic" in am_class or (cadd is not None and cadd >= 28.0) or (vest is not None and vest >= 0.80):
        return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-extrabold bg-red-100 text-red-900 border border-red-300 shadow-sm" title="High Pathogenicity Predictor Consensus">'
                '🔴 Highly Pathogenic</span>', 'highly-pathogenic')

    if "likely_pathogenic" in cv_sig or "likely pathogenic" in cv_sig or (revel is not None and revel >= 0.50) or (am_score is not None and am_score >= 0.50) or (cadd is not None and cadd >= 20.0) or (chasm is not None and chasm >= 0.50):
        return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-bold bg-orange-100 text-orange-900 border border-orange-300" title="Likely Pathogenic / Moderate Predictor Hit">'
                '🟠 Likely Pathogenic</span>', 'likely-pathogenic')

    if any(k in cv_sig for k in ("risk", "drug", "assoc", "affecting")) or ev.get("gwas_disease") or r.get("gwas_disease"):
        return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-semibold bg-sky-100 text-sky-900 border border-sky-300" title="Risk Factor / GWAS Association">'
                '🔵 Risk Factor / GWAS</span>', 'risk-factor')

    if "benign" in cv_sig or (revel is not None and revel <= 0.25) or "benign" in am_class:
        return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-medium bg-emerald-100 text-emerald-900 border border-emerald-300" title="Benign / Likely Benign">'
                '🟢 Likely Benign</span>', 'likely-benign')

    return ('<span class="px-2.5 py-0.5 rounded text-[11px] font-semibold bg-amber-100 text-amber-900 border border-amber-300" title="Variant of Uncertain Significance">'
            '🟡 Uncertain (VUS)</span>', 'vus')
...
def _gene_card(hugo, variants):
    ...
    for idx, r in enumerate(variants, 1):
        ...
        zyg = ev.get("zygosity")
        vaf = ev.get("vaf")
        path_badge, path_cls = _pathogenicity_spectrum_badge(r)
        ...
        revel_v = r.get("revel")
        am_v = r.get("am_path") or r.get("alphamissense")
        cadd_v = r.get("cadd_phred")
        chasm_v = r.get("chasmplus") or r.get("chasmplus_score")
        vest_v = r.get("vest4") or r.get("vest4_score")
        cv_sig_v = ev.get("clinvar_class") or r.get("clinvar_sig")
        
        chips = []
        if cv_sig_v: chips.append(f'<span class="font-bold text-slate-800">ClinVar:</span> {rr._clinvar_link(r.get("clinvar_id"), cv_sig_v)}')
        if revel_v is not None and str(revel_v) not in ('-', 'None', ''): chips.append(f'<span class="font-bold text-slate-800">REVEL:</span> {revel_v}')
        if am_v is not None and str(am_v) not in ('-', 'None', ''): chips.append(f'<span class="font-bold text-slate-800">AlphaMissense:</span> {am_v}')
        if cadd_v is not None and str(cadd_v) not in ('-', 'None', ''): chips.append(f'<span class="font-bold text-slate-800">CADD:</span> {cadd_v}')
        if chasm_v is not None and str(chasm_v) not in ('-', 'None', ''): chips.append(f'<span class="font-bold text-slate-800">CHASMplus:</span> {chasm_v}')
        if vest_v is not None and str(vest_v) not in ('-', 'None', ''): chips.append(f'<span class="font-bold text-slate-800">VEST4:</span> {vest_v}')
        
        chips_html = " &nbsp;|&nbsp; ".join(chips) if chips else "No predictor scores catalogued"

        var_blocks.append(f"""
        <div class="variant-compact-card path-{path_cls}" style="margin-top:10px; background:#ffffff; border-radius:10px; border:1px solid #cbd5e1; overflow:hidden; box-shadow:0 1px 2px rgba(0,0,0,0.04);">
          <!-- Compact Main Header Bar -->
          <div class="v-main-bar" style="padding:10px 14px; display:flex; flex-wrap:wrap; align-items:center; gap:8px; background:#f8fafc; border-bottom:1px solid #e2e8f0;">
            <span class="font-extrabold text-slate-800" style="font-size:12px;">#{idx}</span>
            {path_badge}
            {_phase_badge(ev)}
            <span class="font-mono text-slate-700 font-bold" style="font-size:12.5px;">{html.escape(str(r.get('chrom','')))}:{html.escape(str(r.get('pos','')))} {html.escape(rr._fmt_allele(r.get('ref')))}&gt;{html.escape(rr._fmt_allele(r.get('alt')))}</span>
            <span class="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-200 text-slate-800">{html.escape(so)}</span>
            <span class="font-mono text-purple-900 font-bold text-xs">{html.escape(rr._fmt_allele(r.get('achange') or r.get('cchange') or '', 24))}</span>
            <span class="text-xs text-slate-600 font-mono"><b>{rr._fmt_af(vaf) if vaf is not None else '-'}</b> ({depth_str} Reads)</span>
            <span class="font-mono font-bold text-[11px] text-slate-700 bg-white px-2 py-0.5 rounded border border-slate-300 ml-auto" style="margin-left:auto;">{qual_str}</span>
            <button onclick="this.closest('.variant-compact-card').querySelector('.v-drawer').classList.toggle('hidden')" style="font-size:11px; font-weight:700; color:#2563eb; background:#eff6ff; border:1px solid #bfdbfe; padding:2px 8px; border-radius:6px; cursor:pointer;">Metrics & Drawer ▾</button>
          </div>

          <!-- Predictors Quick Summary Bar -->
          <div style="padding:6px 14px; font-size:11.5px; background:#ffffff; border-bottom:1px solid #f1f5f9; color:#475569;">
            {chips_html}
          </div>
          
          <!-- Collapsible Metrics Drawer -->
          <div class="v-drawer hidden" style="padding:12px 14px; background:#f8fafc; border-top:1px solid #e2e8f0;">
            <div class="grid" style="margin:0 0 10px 0;">
              <div><label>Zygosity</label>{html.escape(zyg or '-')}</div>
              <div><label>Variant Allele Frac</label><strong>{rr._fmt_af(vaf) if vaf is not None else '-'}</strong><div style="font-size:11px; color:#64748b; font-family:monospace; margin-top:2px;">{depth_str} Reads</div></div>
              <div><label>dbSNP</label>{rsid_html}</div>
              <div><label>gnomAD4 AF</label>{rr._fmt_af(r.get('gnomad4_af'))}</div>
              <div><label>All of Us AF</label>{rr._fmt_af(r.get('allofus_af'))}</div>
              <div><label>ClinVar</label>{rr._clinvar_link(r.get('clinvar_id'), r.get('clinvar_sig'))}</div>
