...
COMP_HTML="$OUTDIR/${PREFIX}_ontology_domain_comparison_report.html"

# -----------------------------------------------------------------------------
# Stage 5: Rendering Multi-Domain Master Deliverables & Comparison Report
# -----------------------------------------------------------------------------
echo "[6/6] Rendering deliverables (Universal Master Hub + Domain Comparison Report)..."
case "$RENDERER" in
    master|hub|portal)
...
echo "[6b/6] Rendering side-by-side Domain Comparison Report (Cardio vs Autoimmune)..."
python3 lib/render_domain_comparison.py \
    --in-json "$ACT_JSON" --out-html "$COMP_HTML" --domain-config "$CONFIG" || true
...
(cd "$OUTDIR" && zip -q -r "$ZIP_OUT" "$(basename "$HTML")" "$(basename "$COMP_HTML")" "$(basename "$TSV")" "$(basename "$TEXT")") || true
python3 "$SCRIPT_DIR/cloud_delivery_service.py" "$HTML" "Ontology" || true
python3 "$SCRIPT_DIR/cloud_delivery_service.py" "$COMP_HTML" "Ontology" || true
...
echo "  Domain Comparison Report HTML: $COMP_HTML"
